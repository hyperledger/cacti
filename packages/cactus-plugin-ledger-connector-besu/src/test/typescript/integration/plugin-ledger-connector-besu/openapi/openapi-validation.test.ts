...
describe("PluginLedgerConnectorBesu", () => {
  ...
  test(`${testCase} - ${fDeploy} - ${cWithoutParams}`, async () => {
    ...
    await expect(apiClient.deployContractSolBytecodeV1(
      parameters as unknown as DeployContractSolidityBytecodeV1Request,
    )).rejects.toMatchObject({
      response: {
        status: 400,
        data: expect.arrayContaining([
          expect.objectContaining({ path: expect.stringContaining("/body/contractName") }),
          expect.objectContaining({ path: expect.stringContaining("/body/bytecode") }),
          expect.not.objectContaining({ path: expect.stringContaining("/body/gas") })
        ])
      }
    });
  ...
  test(`${testCase} - ${fInvoke} - ${cInvalidParams}`, async () => {
    ...
    await expect(apiClient.invokeContractV1(parameters as InvokeContractV1Request))
      .rejects.toMatchObject({
        response: {
          status: 400,
          data: expect.arrayContaining([
            expect.objectContaining({ path: expect.stringContaining("/body/fake") })
          ])
        }
      });
  ...
  test(`${testCase} - ${fRun} - ${cWithoutParams}`, async () => {
    ...
    await expect(apiClient.runTransactionV1(parameters as RunTransactionRequest))
      .rejects.toMatchObject({
        response: {
          status: 400,
          data: expect.arrayContaining([
            expect.objectContaining({ path: expect.stringContaining("/body/consistencyStrategy") })
          ])
        }
      });
  ...
  test(`${testCase} - ${fSign} - ${cWithoutParams}`, async () => {
    ...
    const parameters = {
      keychainRef: keychainRefForSigned,
      transactionHash: (runTxRes.data as any).data.transactionReceipt.transactionHash,
    };
    
    await expect(apiClient.signTransactionV1(parameters as SignTransactionRequest))
      .rejects.toMatchObject({
        response: {
          status: 400,
          data: expect.arrayContaining([
            expect.objectContaining({ path: expect.stringContaining("/body/keychainId") })
          ])
        }
      });
  ...
  test(`${testCase} - ${fBalance} - ${cWithoutParams}`, async () => {
    ...
    await expect(apiClient.getBalanceV1(parameters as GetBalanceV1Request))
      .rejects.toMatchObject({
        response: {
          status: 400,
          data: expect.arrayContaining([
  ...
  test(`${testCase} - ${fBlock} - ${cWithoutParams}`, async () => {
    ...
    await expect(apiClient.getBlockV1(parameters as GetBlockV1Request))
      .rejects.toMatchObject({
        response: {
          status: 400,
          data: expect.arrayContaining([
            expect.objectContaining({ path: expect.stringContaining("/body/blockHashOrBlockNumber") })
          ])
        }
      });
  ...
  test(`${testCase} - ${fPastLogs} - ${cWithoutParams}`, async () => {
    ...
    try {
      const parameters = {};
      const response = await apiClient.getPastLogsV1(parameters as GetPastLogsV1Request);
  ...
  test(`${testCase} - ${fRecord} - ${cWithoutParams}`, async () => {
    try {
      const parameters = {};
      const response = await apiClient.getBesuRecordV1(parameters as GetBesuRecordV1Request);
      console.log("e.response.status should be 400 but actually is,", response.status)
    } catch (e) {
      expect(e.response.status).toEqual(400);
      const fields = e.response.data.map((param: any) =>
