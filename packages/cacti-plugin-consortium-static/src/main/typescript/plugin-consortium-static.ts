...
import { NewConsortiumRepository } from "./repository/new-consortium-repository";
  ...
  implements ICactusPlugin, IPluginWebService
{
  ...
  private readonly repo: NewConsortiumRepository;
  ...
  constructor(public readonly options: IPluginConsortiumStaticOptions) {
    ...
    this.repo = new NewConsortiumRepository({
  ...
  public broadcastJoinRequest(req: NewNodeRequestV1) {
    ...
    nodes.forEach(async (node) => {
      ...
      const sig = await this.sign(safeStableStringify(message));
  ...
  async verifySignature(
    ...
    const payload1 = JSON.parse(payload.toString());
    const signedData = new Uint8Array(payload1.data);
